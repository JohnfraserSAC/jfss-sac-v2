-- =========================================================
-- Owner supervisor requests: limits, duplicates, self-review block
-- =========================================================

create or replace function public.count_active_club_advisors(
  p_club_id uuid,
...
revoke all on function public.count_active_club_advisors(uuid, text)
from public;
grant execute on function public.count_active_club_advisors(uuid, text)
to authenticated;


create or replace function public.count_pending_supervisor_emails(
  p_club_id uuid,
  p_school_year text default null
)
returns integer
...
set search_path = ''
as $$
  select count(distinct s.supervisor_email)::integer
  ...
  where r.club_id = p_club_id
    ...
    and not exists (
      ...
      where a.club_id = r.club_id
        and a.school_year = r.school_year
        and a.supervisor_email = s.supervisor_email
        and a.status = 'ACTIVE'
    );
$$;

revoke all on function public.count_pending_supervisor_emails(uuid, text)
from public;
grant execute on function public.count_pending_supervisor_emails(uuid, text)
to authenticated;
...
begin
  ...
  v_active_count := public.count_active_club_advisors(p_club_id, v_year);
  v_pending_count := public.count_pending_supervisor_emails(p_club_id, v_year);

  if v_active_count + v_pending_count + cardinality(v_emails) > 3 then
    raise exception
      'A club may have at most three teacher supervisors, including pending requests';
  end if;
...
begin
  ...
  if v_req.submitted_by = v_user_id then
    raise exception
      'You cannot approve or reject a supervisor request you submitted'
      using errcode = '42501';
  end if;
