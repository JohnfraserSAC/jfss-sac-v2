-- =========================================================
-- Supervisor reject UX + required signature attachment
-- =========================================================
...
begin
  ...
  -- Block self-approval only. Rejection by the submitter (if also SAC_ADMIN)
  -- is allowed so the queue cannot get stuck.
  ...
  if v_action = 'UNDER_REVIEW' then
    ...
    update public.club_supervisor_requests
    set status = 'REJECTED',
        review_notes = coalesce(v_notes, 'Rejected by SAC admin.'),
        reviewed_by = v_user_id,
        reviewed_at = v_now
    where id = p_request_id;
