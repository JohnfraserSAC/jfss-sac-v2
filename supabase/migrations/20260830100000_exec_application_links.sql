-- Add optional executive-application links alongside member-application links.

alter table public.clubs
  add column if not exists exec_application_url text;
...
create or replace function public.submit_club_registration_application_with_application_urls(
  p_request_id uuid,
...
begin
  ...
  v_url := nullif(btrim(coalesce(p_exec_application_url, '')), '');
  if v_url is not null
     and v_url !~ '^https?://[^[:space:]]+$' then
    raise exception 'Enter a valid executive application link.';
  end if;

  v_request_id := public.submit_club_registration_application_with_member_url(
    p_request_id,
    p_proposed_name,
    p_description,
    p_student_benefit,
  ...
  set exec_application_url = v_url
...
revoke all on function public.submit_club_registration_application_with_application_urls(
  uuid, text, text, text, text, text[], text, text, text, text, text[],
  text, text, text, text, text, text, text
) from public, anon;

grant execute on function public.submit_club_registration_application_with_application_urls(
  uuid, text, text, text, text, text[], text, text, text, text, text[],
  text, text, text, text, text, text, text
) to authenticated;

create or replace function public.submit_club_reapplication_with_application_urls(
  p_request_id uuid,
  p_club_id uuid,
...
begin
  ...
  v_request_id := public.submit_club_reapplication_with_member_url(
    p_request_id,
    p_club_id,
    p_short_description,
    p_description,
...
revoke all on function public.submit_club_reapplication_with_application_urls(
  uuid, uuid, text, text, text, text, text, text[], text, text, text,
  boolean, boolean, jsonb, jsonb, text, text
) from public, anon;

grant execute on function public.submit_club_reapplication_with_application_urls(
  uuid, uuid, text, text, text, text, text, text[], text, text, text,
  boolean, boolean, jsonb, jsonb, text, text
) to authenticated;

create or replace function public.update_club_reapplication_with_application_urls(
  p_request_id uuid,
  p_short_description text,
  p_description text,
  p_public_email text,
...
begin
  ...
  v_request_id := public.update_club_reapplication_with_member_url(
    p_request_id,
    p_short_description,
    p_description,
    p_public_email,
...
revoke all on function public.update_club_reapplication_with_application_urls(
  uuid, text, text, text, text, text, text[], text, text, text, boolean,
  boolean, jsonb, jsonb, text, text
) from public, anon;

grant execute on function public.update_club_reapplication_with_application_urls(
  uuid, text, text, text, text, text, text[], text, text, text, boolean,
  boolean, jsonb, jsonb, text, text
) to authenticated;

create or replace function public.update_owned_club_profile_with_application_urls(
  p_club_id uuid,
  p_name text,
  p_description text,
...
begin
  ...
  v_row := public.update_owned_club_profile_with_member_url(
    p_club_id,
    p_name,
    p_description,
    p_contact_email,
...
revoke all on function public.update_owned_club_profile_with_application_urls(
  uuid, text, text, text, text, text, text, text, text[], text, text, text,
  text
) from public, anon;

grant execute on function public.update_owned_club_profile_with_application_urls(
  uuid, text, text, text, text, text, text, text, text[], text, text, text,
  text
) to authenticated;

-- Keep the executive link in sync when a re-application is approved.
...
-- Keep both application links in sync when a new club is approved.
