-- Event proposals use a start and end date.
...
alter table public.club_event_requests
  drop constraint if exists club_event_end_date_valid;

alter table public.club_event_requests
  add constraint club_event_end_date_valid
...
revoke all on function public.submit_club_event_request(
  uuid, uuid, text, text, date, date, text, text, text
) from public, anon;
