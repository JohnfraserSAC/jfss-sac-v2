-- Restore historical Fraser Chefs identity (no duplicate, no active roles).
-- Existing club id: adbff931-c030-4ac5-9ed6-78b9105a3dbd
...
begin
  ...
  if v_count > 1 then
    raise exception 'Multiple Fraser Chefs club rows exist (%). Resolve duplicates before restore.', v_count;
  end if;
  ...
  if v_club_id is null then
    ...
    -- Historical seed-style identity (not publicly active without annual ACTIVE).
  ...
  -- Keep membership history, but never leave active roles.
  update public.club_memberships
  set status = 'INACTIVE'
  where club_id = v_club_id
    and status = 'ACTIVE';

  -- An APPROVED reapplication for the current year blocks the selector.
  -- Preserve the row; mark it rejected so a new reapplication can be submitted.
